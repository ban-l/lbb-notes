...
public class SelectorImplementation {
    ...
    // 不同平台的SelectorProvider
    public static void checkSelectorProvider() {
        SelectorProvider provider = SelectorProvider.provider();
        System.out.println("Selector提供者: " + provider.getClass().getName());
        System.out.println("操作系统: " + System.getProperty("os.name"));
        
        // 不同平台的实现：
        // - Linux: EPollSelectorProvider (使用epoll)
        // - macOS: KQueueSelectorProvider (使用kqueue)
        // - Windows: WindowsSelectorProvider (使用IOCP)
        // - 其他: PollSelectorProvider (使用poll)
    }
    ...
    public static void openSelector() throws IOException {
        ...
        // 方式3: 使用特定的Provider类
        if (provider instanceof sun.nio.ch.EPollSelectorProvider) {
            System.out.println("使用epoll实现");
        } else if (provider instanceof sun.nio.ch.KQueueSelectorProvider) {
            System.out.println("使用kqueue实现");
        } else if (provider instanceof sun.nio.ch.WindowsSelectorProvider) {
            System.out.println("使用IOCP实现");
        }
        
        // Selector内部结构
        System.out.println("Selector是否打开: " + selector1.isOpen());
    }
    
    // Selector的线程安全性
    public static class SelectorThreadSafety {
        private final Selector selector;
        private final Object selectorLock = new Object();
        
        public SelectorThreadSafety() throws IOException {
            this.selector = Selector.open();
        }
        
        // 安全的注册方法
        public void registerChannel(SelectableChannel channel, int ops) 
            throws ClosedChannelException {
            synchronized (selectorLock) {
                selector.wakeup(); // 唤醒正在select的线程
                channel.register(selector, ops);
            }
        }
        
        // 安全的选择操作
        public int selectSafely() throws IOException {
            synchronized (selectorLock) {
