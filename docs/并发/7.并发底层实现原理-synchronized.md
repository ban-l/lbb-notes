...
**目标**：在无实际竞争或短时间竞争(多线程交替执行)场景下，避免使用操作系统互斥量（重量级锁），减少用户态与内核态的切换开销。  
...
| 锁状态       | 比喻                                                         | 特点                                                  |
| ------------ | ------------------------------------------------------------ | :---------------------------------------------------- |
| **偏向锁**   | “这个座位我预定了，以后我来都坐这，别人别动。”               | 针对一个线程，消除同步开销。                          |
| **轻量级锁** | “这个座位现在是空的，我用我的杯子占个座（CAS）。如果有人也想来坐，我们商量一下（自旋），看谁先来。” | 针对多个线程交替使用，通过CAS和自旋避免系统调用。     |
| **重量级锁** | “这个座位现在是我的，我去睡觉了（线程阻塞）。你们所有人都去后面排队等着，我醒了叫你们（操作系统调度）。” | 针对高强度竞争，未获取锁的线程直接挂起，避免空耗CPU。 |
